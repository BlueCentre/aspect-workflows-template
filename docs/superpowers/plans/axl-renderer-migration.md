...
- [ ] **Phase 4 — Big subsystems:**
  ...
  - [x] E1/E2: Pulumi repo_config + bazel-wrapped pulumi tooling + create-app. DONE. 13 files ported, shipped unconditionally (no flag, matching old design). `rules_shell` made **unconditional** (the `pulumi_project` macro emits sh_binary wrappers); the 3 wrapper scripts marked `executable` (sh_binary requires +x). Root `# gazelle:exclude infrastructure` + `# gazelle:exclude tools/pulumi` so gazelle leaves the standalone Go module + hand-authored macro alone. Verified: py renders the tooling, `//...` builds (200 actions), gazelle leaves pulumi dirs untouched. (E3 template-repo infra is Phase 6 tooling.)
