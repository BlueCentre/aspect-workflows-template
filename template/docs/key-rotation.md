...
(`bazel run //tools/remote:setup`; see [remote-build.md](remote-build.md)){% if copybara %}, plus the
Copybara sync secrets{% endif %}. The same mechanics apply to any credential stored
...
> the remote-build helper and enable RBE in CI{% if copybara %}, or wire up Copybara sync{% endif %}.

The detailed steps below are written for `BUILDBUDDY_API_KEY`.{% if copybara %} For the
Copybara secrets, only **how you create the new credential** differs — see the
brief section at the end.{% endif %}
